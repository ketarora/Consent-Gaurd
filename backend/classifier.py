...
Uses Claude Haiku 4.5 to classify messages against four CCPA dark-pattern
categories: confirm_shaming, forced_continuity, drip_pricing, basket_sneaking.
Also serves as a fallback for ambiguous false-urgency cases that the regex
pre-filter can't confidently resolve.
...
async def classify_message(text: str) -> Optional[Flag]:
    """
    Classify a message using Claude Haiku 4.5.

    Sends the message to the Anthropic API and parses the structured
    ...
    if not api_key:
        logger.warning("ANTHROPIC_API_KEY not set — classifier cannot run, failing safe to held.")
        return _make_classifier_error_flag(text, "API key not configured")
    ...
    for attempt in range(2):
        try:
            ...
            logger.warning(f"Classifier attempt {attempt + 1} failed: {e}")
            if attempt == 0:
                continue  # Retry once.
...
async def _call_anthropic(api_key: str, model: str, text: str) -> Optional[Flag]:
    ...
    if quoted_span.lower() not in text.lower():
        quoted_span = text[:80] + ("..." if len(text) > 80 else "")
...
def _make_classifier_error_flag(text: str, error_reason: str) -> Flag:
    ...
    return Flag(
        ...
        quoted_span=span,
        cleared_by_allowlist=False,
        suggested_rewrite="[Compliance System Offline] Agent payload halted. Please provide a manual rewrite."
    )
